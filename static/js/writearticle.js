...
// const openArticleBtns = document.querySelectorAll('#WriteArticleBtn, #openWriteArticle');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modal = document.getElementById('writeArticleModal');
    if (!modal) return; // stop if modal not on page
    ...
    const imageContainer = document.getElementById('selectedImageContainer');
    const draftIdInput = document.getElementById('draftIdInput');
    const alertBox = document.getElementById('hiddenArea');
    const closeBtn = document.getElementById('modalCloseBtn');

    // Support multiple triggers: sidebar button & profile page box
    const openArticleBtns = document.querySelectorAll('#WriteArticleBtn, #openWriteArticle');
...
openArticleBtns.forEach(btn => {
    btn.addEventListener('click', async () => {
       ...
       try {
    const res = await fetch('/articles/get-latest-draft/');
    ...
    if (data.success) {
        ...
        populateImagesFromDraftDB(data.images);
    } else {
        resetFormFields();
    }
} catch (err) {
...
closeBtn.addEventListener('click', () => {
    ...
    const showSelectedImages = (files) => {
        imageContainer.innerHTML = "";
        if (!files.length) { imageContainer.style.display = 'none'; return; }
        imageContainer.style.display = 'flex';
        imageContainer.style.flexWrap = 'wrap';
        imageContainer.style.gap = '10px';
        ...
        Array.from(files).forEach((file, index) => {
            ...
            reader.onload = (e) => {
                ...
                removeBtn.addEventListener('click', () => {
                    const dt = new DataTransfer();
                    Array.from(photoInput.files).filter((_, i) => i !== index).forEach(f => dt.items.add(f));
                    photoInput.files = dt.files;
                    showSelectedImages(photoInput.files);
    ...
    photoInput.addEventListener('change', () => { if (photoInput.files.length) showSelectedImages(photoInput.files); });

   
    // Right after your current populateImagesFromDraft function
const populateImagesFromDraftDB = (images) => {
    imageContainer.innerHTML = "";
    ...
    images.forEach((url, index) => {
        ...
        removeBtn.addEventListener('click', () => {
            // Optional: just remove from DOM for now
            div.remove();
        });
...
formData.append('draft_id', draftIdInput.value);
            ...
            .then(data => {
                if (data && data.success) {
    draftIdInput.value = data.draft_id || draftIdInput.value;


      showAlert('Draft saved', 'success', 1200);
...
else showAlert('Error saving draft', 'danger', 2400);
            })
            .catch(() => showAlert('Error saving draft', 'danger', 2400));
        });
    }
...
formData.append('draft_id', draftIdInput.value);
    ...
    /* =========================
       Alert helper
    ========================== */
    const showAlert = (msg, type = 'success', timeout = 2200) => {
        if (!alertBox) return;
        alertBox.innerHTML = `<div class="alert alert-${type} alert-dismissible fade show" role="alert">${msg}</div>`;
        alertBox.style.display = 'block';
        if (timeout) setTimeout(() => { alertBox.style.display='none'; alertBox.innerHTML=''; }, timeout);
    };
});
